...
test("cfa filters smoke", async ({ page }) => {
  ...
  try {
    ...
    const raceOptions = await page.$$eval("#filter-race option", (opts) =>
      opts.map((o) => `${o.value}|${(o.textContent || "").trim()}`)
    );
    const stageOptionsInitial = await page.$$eval("#filter-stage option", (opts) =>
      opts.map((o) => `${o.value}|${(o.textContent || "").trim()}`)
    );
  ...
  console.log('AFTER_RESET', minAfterReset, rowsAfterReset);
    ...
    const stageOptionsAfterRace = await page.$$eval("#filter-stage option", (opts) =>
      opts.map((o) => `${o.value}|${(o.textContent || "").trim()}`)
    );
